...
class BaseProxyMethod:
    ...
    col_policy : string, optional
        Name of the column that indicates treatment policy (probability of
        treatment). If not given, it is assumed that application of treatment
        was randomly assigned with the same probability across the entire
        population.
    ...
    def shuffle_fit(self, iterations=10, n_bins=20, params=None, transform_train=None, clear=False, plot_type='cgains', **kwargs):
        ...
        for seed in range(iterations):
            df_train, df_test = train_test_split(self.df, test_size=0.2, random_state=seed)
            if transform_train:
